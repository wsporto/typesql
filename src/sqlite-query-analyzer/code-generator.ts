...
import {
	...
	type TsField2,
	mapToTsRelation2
} from '../ts-nested-descriptor';
import { preprocessSql } from '../describe-query';
import { explainSql } from './query-executor';
import {
	mapToDynamicParams,
	mapToDynamicResultColumns,
	mapToDynamicSelectColumns
} from '../ts-dynamic-query-descriptor';
...
function mapToColumnInfo(col: ColumnSchema, checkOptional: boolean) {
	const defaultValue =
		col.columnKey === 'PRI' && col.column_type === 'INTEGER'
			? 'AUTOINCREMENT'
			: col.defaultValue;
...
export function generateCrud(
	...
	const columnInfo = columns.map((col) =>
		mapToColumnInfo(col, queryType === 'Insert' || queryType === 'Update')
	);
	...
	const params = columnInfo.map((col) =>
		mapColumnToTsFieldDescriptor(col, client)
	);
	...
	const tsDescriptor: TsDescriptor = {
		...
		data:
			queryType === 'Update'
				? params.filter((param) => param.name !== keyColumnInfo[0]?.name)
				: []
	};

	const queryName = getQueryName(queryType, tableName);

	const code = generateCodeFromTsDescriptor(
		client,
		queryName,
		tsDescriptor,
		true,
...
function createTsDescriptor(
	...
	const tsDescriptor: TsDescriptor = {
		...
		parameterNames: [],
		parameters: queryInfo.parameters.map((param) =>
			mapParameterToTsFieldDescriptor(param, client)
		),
		data: queryInfo.data?.map((param) =>
			mapParameterToTsFieldDescriptor(param, client)
		),
		...
		const nestedDescriptor2 = queryInfo.nestedInfo.map((relation) => {
			const tsRelation: RelationType2 = {
				...
				fields: relation.fields.map((field) =>
					mapFieldToTsField(queryInfo.columns, field, client)
				),
				relations: relation.relations.map((relation) =>
					mapToTsRelation2(relation)
				)
			};
			return tsRelation;
		});
...
function mapColumns(
	...
	const escapedColumnsNames = renameInvalidNames(
		columns.map((col) => col.columnName)
	);
	return columns.map((col, index) =>
		mapColumnToTsFieldDescriptor(
			{ ...col, columnName: escapedColumnsNames[index] },
			client
		)
	);
}

function mapFieldToTsField(
	columns: ColumnInfo[],
	field: Field2,
	client: SQLiteClient
): TsField2 {
...
function mapParameterToTsFieldDescriptor(
	col: ParameterDef,
	client: SQLiteClient
) {
...
function generateCodeFromTsDescriptor(
	client: SQLiteClient,
	queryName: string,
	...
	const paramsTypes = removeDuplicatedParameters2(
		tsDescriptor.dynamicQuery2 == null
			...
			: mapToDynamicParams(tsDescriptor.parameters)
	);
	...
	const allParameters = (
		tsDescriptor.data?.map((param) => fromDriver('data', param)) || []
	...
	if (tsDescriptor.dynamicQuery2 != null) {
		...
		writeTypeBlock(
			...
			paramsTypeName,
			false,
			tsDescriptor.dynamicQuery2 ? undefined : orderByField
		);
		const resultTypes =
			tsDescriptor.dynamicQuery2 == null
				? tsDescriptor.columns
				: mapToDynamicResultColumns(tsDescriptor.columns);
		...
		writer.write(`export type ${whereTypeName} =`).indent(() => {
			tsDescriptor.columns.forEach((col) => {
				writer.writeLine(
					`| ['${col.name}', ${getOperator(col.tsType)}, ${col.tsType} | null]`
				);
				writer.writeLine(`| ['${col.name}', SetOperator, ${col.tsType}[]]`);
				writer.writeLine(
					`| ['${col.name}', BetweenOperator, ${col.tsType} | null, ${col.tsType} | null]`
				);
			});
		});
		writer.blankLine();
		const asyncModified = client === 'libsql' ? 'async ' : '';
		const returnTypeModifier =
			client === 'libsql' ? `Promise<${returnType}>` : returnType;
		writer
			.write(
				`export ${asyncModified}function ${camelCaseName}(${functionArguments}): ${returnTypeModifier}`
			)
			.block(() => {
				writer
				...
				if (hasCte) {
					...
					tsDescriptor.dynamicQuery2?.with.forEach((withFragment, index) => {
						const selectConditions = withFragment.dependOnFields.map(
							(fieldIndex) =>
								`params.select.${tsDescriptor.columns[fieldIndex].name}`
						);
						if (selectConditions.length > 0) {
							selectConditions.unshift('params?.select == null');
						}
						const whereConditions = withFragment.dependOnFields.map(
							(fieldIndex) =>
								`where.${tsDescriptor.columns[fieldIndex].name} != null`
						);
						const orderByConditions =
							withFragment.dependOnOrderBy?.map(
						...
						const paramValues = withFragment.parameters.map(
							(param) => `params?.params?.${param}`
						);
						writer
							.write(`if (${allConditions.join(`${EOL}\t|| `)})`)
							.block(() => {
								writer.write(
									`withClause += EOL + \`${withFragment.fragment}\`;`
								);
				...
				tsDescriptor.dynamicQuery2?.select.forEach((select, index) => {
					writer
						.write(
							`if (params?.select == null || params.select.${tsDescriptor.columns[index].name})`
						)
						.block(() => {
							writer.writeLine(
								`sql = appendSelect(sql, \`${select.fragment}\`);`
							);
							select.parameters.forEach((param) => {
								writer.writeLine(
									`paramsValues.push(params?.params?.${param} ?? null);`
								);
							});
						});
				});
				...
				tsDescriptor.dynamicQuery2?.from.forEach((from, index) => {
					if (index === 0) {
						...
						const selectConditions = from.dependOnFields.map(
							(fieldIndex) =>
								`params.select.${tsDescriptor.columns[fieldIndex].name}`
						);
						if (selectConditions.length > 0) {
						...
						const whereConditions = from.dependOnFields.map(
							(fieldIndex) =>
								`where.${tsDescriptor.columns[fieldIndex].name} != null`
						);
						const orderByConditions =
							from.dependOnOrderBy?.map(
								(orderBy) => `orderBy['${orderBy}'] != null`
							) || [];
						const allConditions = [
							...selectConditions,
						...
						const paramValues = from.parameters.map(
							(param) => `params?.params?.${param}`
						);
						writer
							.write(`if (${allConditions.join(`${EOL}\t|| `)})`)
				...
				tsDescriptor.dynamicQuery2?.where.forEach((fragment) => {
					const paramValues = fragment.parameters.map(
				...
				writer.write('params?.where?.forEach(condition => ').inlineBlock(() => {
					...
					tsDescriptor.dynamicQuery2?.select.forEach((select, index) => {
						if (select.parameters.length > 0) {
							writer
								.write(
									`if (condition[0] == '${tsDescriptor.columns[index].name}')`
								)
								.block(() => {
									select.parameters.forEach((param) => {
										writer.writeLine(
				...
				if (client === 'better-sqlite3') {
					...
					writer
						.indent()
						.write(
							`.map(data => mapArrayTo${resultTypeName}(data, params?.select))${tsDescriptor.multipleRowsResult ? '' : '[0]'};`
						);
				}
				if (client === 'libsql') {
					writer
		...
		writer
			.write(
				`function mapArrayTo${resultTypeName}(data: any, select?: ${selectColumnsTypeName})`
			)
			.block(() => {
				...
				tsDescriptor.columns.forEach((tsField) => {
					writer
						...
						.block(() => {
							...
							writer.writeLine(
								`result.${tsField.name} = ${toDriver('data[rowIndex]', tsField)};`
							);
						});
				});
		...
		writer
			.write(
				`function whereConditionsToObject(whereConditions?: ${whereTypeName}[])`
			)
			.block(() => {
		...
		if (orderByField != null) {
			...
			writer
				.write(
					`function orderByToObject(orderBy: ${dynamicParamsTypeName}['orderBy'])`
				)
				.block(() => {
					writer.writeLine('const obj = {} as any;');
		...
		writer
			.write(
				`function whereCondition(condition: ${whereTypeName}): WhereConditionResult | undefined `
			)
			.block(() => {
				writer.blankLine();
				writer.writeLine(
		...
		if (uniqueUpdateParams.length > 0) {
			...
			writer.write(`export type ${dataTypeName} =`).block(() => {
				uniqueUpdateParams.forEach((field) => {
					...
					writer.writeLine(
						`${field.name}${optionalOp}: ${field.tsType}${orNull};`
					);
				});
			});
		}
	...
	if (
			...
			writer
				...
				.block(() => {
					...
					writer
						.indent()
						.write(
							`.map(data => mapArrayTo${resultTypeName}(data))${tsDescriptor.multipleRowsResult ? '' : '[0]'};`
						);
				});
		}
		if (!isCrud && client === 'libsql') {
			writer
				.write(
					`export async function ${camelCaseName}(${functionArguments}): Promise<${returnType}>`
				)
				.block(() => {
					const processedSql = tsDescriptor.orderByColumns
						? replaceOrderByParam(sql)
					...
					writer.indent().write('`').newLine();
					const executeParams =
						queryParams !== '' ? `{ sql, args: ${queryParams} }` : 'sql';
					...
					if (queryType === 'Select') {
						...
						if (tsDescriptor.multipleRowsResult) {
							writer
								.indent()
								.write(
									`.then(rows => rows.map(row => mapArrayTo${resultTypeName}(row)));`
								);
						} else {
							writer
								.indent()
	...
	if (
		...
		if (client === 'libsql') {
			writer
				.write(
					`export async function ${camelCaseName}(${functionArguments}): Promise<${resultTypeName}>`
				)
				.block(() => {
	...
	if (
		(queryType === 'Select' || tsDescriptor.returning) &&
		tsDescriptor.dynamicQuery2 == null
	) {
		writer.blankLine();
		writer
			...
			.block(() => {
				writer.write(`const result: ${resultTypeName} = `).block(() => {
					tsDescriptor.columns.forEach((col, index) => {
						const separator =
							index < tsDescriptor.columns.length - 1 ? ',' : '';
						writer.writeLine(
							`${col.name}: ${toDriver(`data[${index}]`, col)}${separator}`
						);
					});
				});
	...
	if (tsDescriptor.orderByColumns) {
		...
		writer.blankLine();
		writer.writeLine(
			`export type ${orderByTypeName} = keyof typeof orderByFragments;`
		);
		writer.blankLine();
		writer
			.write(
				`function escapeOrderBy(orderBy: ${orderByType}['orderBy']): string`
			)
			.block(() => {
				writer.writeLine(
	...
	if (tsDescriptor.nestedDescriptor2) {
		...
		relations.forEach((relation) => {
			...
			writer.write(`export type ${relationType} = `).block(() => {
				const uniqueNameFields = renameInvalidNames(
					relation.fields.map((f) => f.name)
				);
				relation.fields.forEach((field, index) => {
					writer.writeLine(`${uniqueNameFields[index]}: ${field.tsType};`);
				});
				relation.relations.forEach((field) => {
					const nestedRelationType = generateRelationType(
						capitalizedName,
						field.tsType
					);
					const nullableOperator = field.notNull ? '' : '?';
					writer.writeLine(
						`${field.name}${nullableOperator}: ${nestedRelationType};`
					);
				});
			});
		});
			...
			if (index === 0) {
				if (client === 'better-sqlite3') {
					writer
						...
						.block(() => {
							const params =
								tsDescriptor.parameters.length > 0 ? ', params' : '';
							writer.writeLine(
								`const selectResult = ${camelCaseName}(db${params});`
							);
					...
					writer
						.write(
							`export async function ${camelCaseName}Nested(${functionArguments}): Promise<${relationType}[]>`
						)
						.block(() => {
							const params =
								tsDescriptor.parameters.length > 0 ? ', params' : '';
							writer.writeLine(
								`const selectResult = await ${camelCaseName}(client${params});`
							);
							writer.write('if (selectResult.length == 0)').block(() => {
								writer.writeLine('return [];');
							});
...
function writeExecuteCrudBlock(
	...
	switch (queryType) {
		case 'Select':
			return writeExecutSelectCrudBlock(
				client,
				tableName,
				idColumn,
				columns,
		...
		case 'Insert':
			return writeExecuteInsertCrudBlock(
				client,
				tableName,
				paramTypeName,
				resultTypeName,
		...
		case 'Update':
			return writeExecuteUpdateCrudBlock(
				client,
				tableName,
				idColumn,
				dataTypeName,
		...
		case 'Delete':
			return writeExecutDeleteCrudBlock(
				client,
				tableName,
				idColumn,
				queryParams,
...
function writeExecuteInsertCrudBlock(
	...
	writer.blankLine();
	writer.writeLine(
		`const keys = Object.keys(params) as Array<keyof ${paramTypeName}>;`
	);
	writer.writeLine(
...
function writeExecuteUpdateCrudBlock(
	...
	writer.blankLine();
	writer.writeLine(
		`const keys = Object.keys(data) as Array<keyof ${paramTypeName}>;`
	);
	writer.writeLine(
		'const columns = keys.filter(key => data[key] !== undefined);'
	);
	writer.writeLine(
		`const values = columns.map(col => data[col]!).concat(params.${idColumn});`
	);
	writer.blankLine();
...
function writeCollectFunction(
	...
	writer
		.write(
			`function ${collectFunctionName}(selectResult: ${resultTypeName}[]): ${relationType}[]`
		)
		.block(() => {
			const groupBy = columns[relation.groupIndex].name;
			writer.writeLine(
				`const grouped = groupBy(selectResult.filter(r => r.${groupBy} != null), r => r.${groupBy});`
			);
			writer
				...
				.inlineBlock(() => {
					relation.fields.forEach((field, index) => {
						...
						writer.writeLine(
							`${uniqueNameFields[index]}: row[0].${fieldName}!${separator}`
						);
					});
					relation.relations.forEach((fieldRelation) => {
						...
						writer.writeLine(
							`${fieldRelation.name}: collect${relationType}(row)${cardinality},`
						);
					});
				})
				.write('))');
