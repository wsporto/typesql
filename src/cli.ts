...
function parseArgs() {
	return yargs
		...
		.option('config', {
			describe:
				'Database URI to connect to, e.g. -d mysql://user:password@localhost/mydb.',
			demandOption: true,
			type: 'string',
...
async function compile(watch: boolean, config: TypeSqlConfig) {
	...
	const databaseClientResult = await createClient(
		databaseUri,
		dialect,
		attach,
		authToken
	...
	const filesGeneration = sqlFiles.map((sqlFile) =>
		generateTsFile(
			databaseClient,
			sqlFile,
			dbSchema.right,
			isCrudFile(sqlDir, sqlFile)
		)
	);
...
async function writeIndexFile(sqlDir: string) {
	const tsFiles = fs
		...
		.filter(
			(file) =>
				path.basename(file) !== 'index.ts' && path.extname(file) === '.ts'
		);
...
async function writeSql(
	...
	const generatedOk = checkAndGenerateSql(
		client.type,
		filePath,
		stmtType,
		tableName,
...
function checkAndGenerateSql(
	...
	if (columns.length === 0) {
		console.error(
			`Got no columns for table '${tableName}'. Did you type the table name correclty?`
		);
		return false;
	}
...
async function generateCrudTables(
	client: DatabaseClient,
	...
	for (const tableInfo of filteredTables) {
		...
		if (client.type === 'mysql2') {
			...
			checkAndGenerateSql(
				client.type,
				`${filePath}insert-into-${tableName}.sql`,
				'insert',
				tableName,
				columns
			);
			...
			checkAndGenerateSql(
				client.type,
				`${filePath}delete-from-${tableName}.sql`,
				'delete',
				tableName,
				columns
			);
		} else {
			generateAndWriteCrud(
				client.type,
				`${filePath}select-from-${tableName}.ts`,
				'Select',
				tableName,
				dbSchema
			);
			generateAndWriteCrud(
				client.type,
				`${filePath}insert-into-${tableName}.ts`,
				'Insert',
				tableName,
				dbSchema
			);
			...
			generateAndWriteCrud(
				client.type,
				`${filePath}delete-from-${tableName}.ts`,
				'Delete',
				tableName,
				dbSchema
			);
...
async function selectAllTables(
	client: DatabaseClient
): Promise<Either<string, Table[]>> {
	const selectTablesResult = await selectTables(client);
	if (isLeft(selectTablesResult)) {
		return left(
			`Error selecting table names: ${selectTablesResult.left.description}`
		);
	}
