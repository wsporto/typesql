...
export function generateTsCodeForMySQL(
	...
	const orderByField = generateOrderBy
		...
		: undefined;
	const paramsTypes =
		tsDescriptor.dynamicQuery == null
	...
	writeTypeBlock(
		...
		paramsTypeName,
		false,
		tsDescriptor.dynamicQuery ? undefined : orderByField
	);
	const resultTypes =
		tsDescriptor.dynamicQuery == null
	...
	writer
		.write(
			`export async function ${camelCaseName}(${functionArguments}): Promise<${functionReturnType}>`
		)
		.block(() => {
			if (tsDescriptor.dynamicQuery == null) {
				...
				tsDescriptor.dynamicQuery.select.forEach((fragment) => {
					writer
						...
						.block(() => {
							writer.write(
								`sql = appendSelect(sql, \`${fragment.fragment}\`);`
							);
						});
				});
				...
				tsDescriptor.dynamicQuery.where.forEach((fragment) => {
					const ifParamConditions = fragment.dependOnParams.map(
				...
				.block(() => {
					writer.write(`const result: ${resultTypeName} =`).block(() => {
						tsDescriptor.columns.forEach((tsField, index) => {
							writer.writeLine(
								`${tsField.name}: data[${index}]${commaSeparator(tsDescriptor.columns.length, index)}`
							);
						});
					});
	...
	if (generateOrderBy) {
		const orderByColumnsType = tsDescriptor.orderByColumns
			?.map((col) => `'${col}'`)
			.join(' | ');
		writer.blankLine();
		...
		relations.forEach((relation, index) => {
			...
			if (index === 0) {
				...
				writer
					...
					.block(() => {
						...
						writer.writeLine(
							`const selectResult = await ${camelCaseName}(connection${params});`
						);
			...
			writer
				...
				.block(() => {
					...
					writer.writeLine(
						`const grouped = groupBy(selectResult.filter(r => r.${groupKey} != null), r => r.${groupKey});`
					);
					writer.writeLine(
						`return [...grouped.values()].map(r => ${mapFunctionName}(r))`
					);
				});
			writer.blankLine();
			writer
				.write(
					`function ${mapFunctionName}(selectResult: ${resultTypeName}[]): ${relationType}`
				)
				.block(() => {
					...
					writer.write(`const result: ${relationType} = `).block(() => {
						...
						relation.fields.forEach((field, index) => {
							...
							if (field.type === 'field') {
								...
								writer.writeLine(
									`${uniqueNameFields[index]}: firstRow.${fieldName}!${separator}`
								);
							}
							if (field.type === 'relation') {
								const nestedRelationType = generateRelationType(
								...
								writer.writeLine(
									`${field.name}: collect${nestedRelationType}(selectResult)${cardinality}${separator}`
								);
							}
						});
					});
...
function generateDynamicQueryFrom(
	...
	const selectConditions = fragment.dependOnFields.map(
		(fieldIndex) => `params.select.${columns[fieldIndex].name}`
	);
	if (selectConditions.length > 0) {
		selectConditions.unshift('params?.select == null');
	}
	const paramConditions = fragment.dependOnParams.map(
		(param) => `params.params?.${param} != null`
	);
	const whereConditions = fragment.dependOnFields.map(
		(fieldIndex) => `where.${columns[fieldIndex].name} != null`
	);
...
export function generateRelationType(
...
export function generateTsDescriptor(queryInfo: SchemaDef): TsDescriptor {
	const escapedColumnsNames = renameInvalidNames(
		queryInfo.columns.map((col) => col.columnName)
	);
	...
	const escapedParametersNames = renameInvalidNames(
		uniqueParams.map((col) => col.name)
	);
	...
	if (queryInfo.nestedResultInfo) {
		const nestedDescriptor = createNestedTsDescriptor(
			queryInfo.columns,
			queryInfo.nestedResultInfo
		);
...
export function removeDuplicatedParameters(
...
export function removeDuplicatedParameters2(
