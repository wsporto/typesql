...
WHERE :param1 is true OR (:param2 is true OR :param2::bool is null)`;
	...
	it('crud-insert01', async () => {
		const actual = await generateCrud(dialect, 'Insert', 'mytable1', dbSchema);
	...
	it('crud-delete01', async () => {
		const actual = await generateCrud(dialect, 'Delete', 'mytable1', dbSchema);
