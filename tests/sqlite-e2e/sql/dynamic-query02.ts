...
export function dynamicQuery02(db: Database, params?: DynamicQuery02DynamicParams): DynamicQuery02Result[] {

	const { sql, paramsValues } = buildSql(params);
	return db.prepare(sql)
		.raw(true)
		.all(paramsValues)
		.map(data => mapArrayToDynamicQuery02Result(data, params?.select));
}
