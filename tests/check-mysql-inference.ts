...
async function main() {
    ...
    var combinations = allTypes.flatMap(
        (v, i) => allTypes.slice(i + 1).map(w => ({ first: v, second: w }))
    );
    ...
    const resultArray: [{ name: string, type: string }] = result.statement.columns.map((col: { name: string, columnType: number, columnLength: number, flags: number }) => ({ name: col.name, type: convertTypeCodeToMysqlType(col.columnType, col.flags, col.columnLength) }));
